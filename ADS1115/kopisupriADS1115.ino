...
void initADS() {
  if (!ads.begin()) {
    ...
    while (1); // Berhenti di sini jika ADS1115 gagal
  }
  ads.setGain(GAIN_ONE); // Penguatan 1x (0-4.096V)
}
...
float bacaAsapPersen() {
  int16_t adcValue = ads.readADC_SingleEnded(0); // Baca dari A0
  Serial.print("Nilai Analog Asap : ");
  Serial.println(adcValue);
  // Konversi ADC 16-bit (0-32767) ke persen (0-100%)
  float persenAsap = (adcValue / 32767.0) * 100.0;
  if (persenAsap > 100.0) persenAsap = 100.0; // Pastikan max 100%
  if (persenAsap < 0.0) persenAsap = 0.0;     // Pastikan min 0%
  return persenAsap;
}
...
void setup() {
  initSerial();
  initConfigPortal();
  initI2C();       // Inisialisasi I2C dulu
  initLCD();       // Baru LCD
  initADS();       // Lalu ADS1115
  initIO();
  putarLaguEsKrim();
